"""
OmniForge Godot MCP Agent Server
HTTP API that exposes 191+ Godot tools for the VS Code extension.
"""
...
log = logging.getLogger("omniforge-agent")

app = FastAPI(
    title="OmniForge Godot Agent",
    description="MCP-compatible HTTP server exposing 191+ Godot 4 tools",
    version="0.1.0",
)
...
# ---------------------------------------------------------------------------
# Tool registry — maps tool name → (function, category)
# ---------------------------------------------------------------------------
...
async def call_tool(request: ToolCallRequest) -> ToolCallResponse:
    ...
    # Inject shared state if the function expects these params
    import inspect
    ...
    try:
        result = fn(**args)
        log.info("Tool %s → success=%s", request.tool, result.success)
        return result
    except TypeError as e:
        log.error("Tool %s argument error: %s", request.tool, e)
        return ToolCallResponse(success=False, error=f"Argument error: {e}")
    except Exception as e:  # noqa: BLE001
        log.error("Tool %s error: %s", request.tool, e)
...
async def set_godot(body: dict[str, str]) -> dict[str, Any]:
    """Set the Godot executable path and detect version."""
    exe = body.get("executable", "godot4")
    _state["godot_executable"] = exe
    try:
        result = subprocess.run(
            [exe, "--headless", "--version"],
...
def main() -> None:
    parser = argparse.ArgumentParser(description="OmniForge Godot MCP Agent")
    parser.add_argument("--host", default="127.0.0.1")
    parser.add_argument("--port", type=int, default=9876)
    parser.add_argument("--project", default="", help="Godot project path")
    parser.add_argument("--godot", default="godot4", help="Godot executable")
    ...
    log.info("OmniForge Godot Agent starting on %s:%d", args.host, args.port)
    log.info("Project: %s | Godot: %s | Tools: %d", args.project or "(none)", args.godot, len(TOOL_REGISTRY))

    uvicorn.run(
        "omniforge_godot_agent.server:app",
        host=args.host,
        port=args.port,
